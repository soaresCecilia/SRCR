...
contrato(IdAd, IdAda, TipoContrato, TipoProcedimento, Descricao, Valor, Prazo, Local, Data), L),
comprimento(L, 1)).
...
novoAdjudicatario(Id,Nif,Nome, Morada) :- evolucao(adjudicatario(Id,Nif,Nome, Morada)).

novoAdjudicante(Id,Nif,Nome, Morada) :- evolucao(adjudicante(Id,Nif,Nome, Morada)).
...
adjudicanteID(Id, L) :- solucoes(adjudicante(Id,Nif, Nome, Morada), adjudicante(Id,Nif, Nome, Morada), L).

removeAdjudicante(Id) :- adjudicanteID(Id,[X|_]), involucao(X).



adjudicatarioID(Id, L) :- solucoes(adjudicatario(Id,Nif, Nome, Morada), adjudicatario(Id,Nif, Nome, Morada), L).
